...
cd /home/student/honeypot_files/
mkdir $name
cd $name/
...
sudo cp -r /var/lib/lxc/$name/rootfs/var/log/.downloads ./containerdownloads/

# Copy mitm and auth logs into current directory
sudo cp /home/student/MITM/$name.log ./
sudo cp /var/lib/lxc/$name/var/log/auth.log ./
